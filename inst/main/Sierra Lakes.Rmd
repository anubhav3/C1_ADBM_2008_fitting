---
title: "Fitting ADBM to Sierra Lakes food web"
author: "Owen Petchey"
...
sierra2008.all.web.info <- readRDS(here("data/Sierra Lakes.web.2008.RDS"))
Plot.matrix(sierra2008.all.web.info$predation.matrix)
box()
```

## Sierra Lakes data from Brose et al (2008)

The Sierra Lakes food web data was downloaded from [here](https://dx.doi.org/10.6084/m9.figshare.c.3298772) and then processed (steps are not shown in this document). The food web is similar to that used in Petchey et al (2008).

```{r warning = FALSE}

sierra.openaccess.all.web.info <- readRDS(here("data/Sierra Lakes.web.openaccess.RDS"))
Plot.matrix(sierra.openaccess.all.web.info$predation.matrix)
box()
```
...
## Fit model to 2008 Sierra Lakes data

Here the outcome is the same as in Petchey et al (2008).

```{r eval = FALSE}
NM.RH.web_2008 <- NM.ratio(sierra2008.all.web.info, ratio_initial_pars)
```
...
sum(sierra2008.all.web.info$predation.matrix)
```



## Fit model to open access Sierra Lakes data

```{r eval = FALSE}
NM.RH.web_openaccess <- NM.ratio(sierra.openaccess.all.web.info, ratio_initial_pars)
```
...
sum(sierra.openaccess.all.web.info$predation.matrix)
```
...
EHL_2008 <- Ratio.allometric.EHL(M = sierra2008.all.web.info$species.sizes,
                           e = 1,
                           r.a = 1, r.b = b_2008,
...
box()
Compare.links(web_2008, sierra2008.all.web.info$predation.matrix)
```
