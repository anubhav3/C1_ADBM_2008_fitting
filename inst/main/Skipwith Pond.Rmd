---
title: "Fitting ADBM to Skipwith Pond food web"
author: "Owen Petchey"
...
```{r}
fw_name <- "Skipwith Pond"
skipwith2008.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.2008.RDS")))
Plot.matrix(skipwith2008.all.web.info$predation.matrix)
box()
```

## Skipwith Pond data from Brose et al. (2008)

The Skipwith Pond food web data was downloaded from [here](https://dx.doi.org/10.6084/m9.figshare.c.3298772) and then processed (steps are not shown in this document). The food web is similar to that used in Petchey et al. (2008).

```{r warning = FALSE}

skipwith.openaccess.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.openaccess.RDS")))
Plot.matrix(skipwith.openaccess.all.web.info$predation.matrix)
box()
```
...
## Fit model to 2008 Skipwith Pond data

Here the outcome is the same as in Petchey et al. (2008).

```{r eval = FALSE}
NM.RH.web_2008 <- NM.ratio(skipwith2008.all.web.info, ratio_initial_pars)
```
...
sum(skipwith2008.all.web.info$predation.matrix)
```



## Fit model to open access Skipwith Pond data

```{r eval = FALSE}
NM.RH.web_openaccess <- NM.ratio(skipwith.openaccess.all.web.info, ratio_initial_pars)
```
...
sum(skipwith.openaccess.all.web.info$predation.matrix)
Compare.links(NM.RH.web_openaccess$web, skipwith.openaccess.all.web.info$predation.matrix)
```
...
EHL_2008 <- Ratio.allometric.EHL(M = skipwith2008.all.web.info$species.sizes,
                           e = 1,
                           r.a = 1, r.b = b_2008,
...
box()
Compare.links(web_2008, skipwith2008.all.web.info$predation.matrix)
```
