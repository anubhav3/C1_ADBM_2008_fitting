---
title: "Fitting ADBM to Mill Stream food web"
author: "Owen Petchey"
...
```{r}
fw_name <- "Mill Stream"
mill2008.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.2008.RDS")))
Plot.matrix(mill2008.all.web.info$predation.matrix)
box()
```

## Mill Stream data from Brose et al. (2008)

The Mill Stream food web data was downloaded from [here](https://dx.doi.org/10.6084/m9.figshare.c.3298772) and then processed (steps are not shown in this document). The food web is same is that used in Petchey et al. (2008).

```{r warning = FALSE}

mill.openaccess.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.openaccess.RDS")))
Plot.matrix(mill.openaccess.all.web.info$predation.matrix)
box()
```
...
## Fit model to 2008 Mill Stream data

Here the outcome is the same as in Petchey et al. (2008).

```{r eval = FALSE}
NM.RH.web_2008 <- NM.ratio(mill2008.all.web.info, ratio_initial_pars)
```
...
sum(mill2008.all.web.info$predation.matrix)
```



## Fit model to open access Mill Stream data

```{r eval = FALSE}
NM.RH.web_openaccess <- NM.ratio(mill.openaccess.all.web.info, ratio_initial_pars)
```
...
sum(mill.openaccess.all.web.info$predation.matrix)
Compare.links(NM.RH.web_openaccess$web, mill.openaccess.all.web.info$predation.matrix)
```
...
EHL_2008 <- Ratio.allometric.EHL(M = mill2008.all.web.info$species.sizes,
                           e = 1,
                           r.a = 1, r.b = b_2008,
...
box()
Compare.links(web_2008, mill2008.all.web.info$predation.matrix)
```
