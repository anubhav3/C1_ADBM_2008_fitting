---
title: "Fitting ADBM to Tuesday Lake food web"
author: "Owen Petchey"
...
```{r}
fw_name <- "Tuesday Lake"
tuesday2008.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.2008.RDS")))
Plot.matrix(tuesday2008.all.web.info$predation.matrix)
box()
```

## Tuesday Lake data from Brose et al. (2008)

The Tuesday Lake food web data was downloaded from [here](https://dx.doi.org/10.6084/m9.figshare.c.3298772) and then processed (steps are not shown in this document). The food web is same that used in Petchey et al (2008).

```{r warning = FALSE}

tuesday.openaccess.all.web.info <- readRDS(here(paste0("data/", fw_name, ".web.openaccess.RDS")))
Plot.matrix(tuesday.openaccess.all.web.info$predation.matrix)
box()
```
...
## Fit model to 2008 tuesday data

Here the outcome is the same as in Petchey et al (2008).

```{r eval = FALSE}
NM.RH.web_2008 <- NM.ratio(tuesday2008.all.web.info, ratio_initial_pars)
```
...
sum(tuesday2008.all.web.info$predation.matrix)
```



## Fit model to open access tuesday data

```{r eval = FALSE}
NM.RH.web_openaccess <- NM.ratio(tuesday.openaccess.all.web.info, ratio_initial_pars)
```
...
sum(tuesday.openaccess.all.web.info$predation.matrix)
Compare.links(NM.RH.web_openaccess$web, tuesday.openaccess.all.web.info$predation.matrix)
```
...
EHL_2008 <- Ratio.allometric.EHL(M = tuesday2008.all.web.info$species.sizes,
                           e = 1,
                           r.a = 1, r.b = b_2008,
...
box()
Compare.links(web_2008, tuesday2008.all.web.info$predation.matrix)
```
